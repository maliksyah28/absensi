...
export default function Home() {
  ...
  const absenceProsess = async () => {
    try {
      const resAbsen = await axiosInstance.get(
        `api/attendances/scan/${inputAbsent}`
      );
      
      if (resAbsen.data.data.checkOut == null) {
        setInputAbsent("") 
        return toast({
          title: `Hi ${resAbsen.data.data.name}, berhasil melakukan check IN`,
          status: "success",
          position: "top",
          duration: 3000,
          isClosable: true,
        ...
        setInputAbsent("") 
        return toast({
          title: `Hi ${resAbsen.data.data.name}, berhasil melakukan check Out`,
          status: "success",
          position: "top",
          duration: 3000,
          isClosable: true,
      ...
      {local?.Token ? (
        <>
          <Stack
            ...
            bgGradient="linear-gradient(90deg, rgba(2,0,36,1) 0%, rgba(24,96,105,0.8575805322128851) 49%, rgba(0,212,255,1) 100%)"
          >
            <Flex p={12} align={"center"} justify={"center"}>
              <Stack spacing={6} w={"full"} maxW={"lg"}>
                ...
                <Button size={"md"} w={"20"}>
                  {" "}
                  <Link to="/HistoryAbsence">DashBoard</Link>
                </Button>
              </Stack>
            </Flex>
                  ...
                  onResult={async (result, error) => {
                    try {
                      if (!!result) {
                        ...
                        console.log(res);
                        if (res.data.data.checkOut == null) {
                          return toast({
                            title: `Hi ${res.data.data.name}, berhasil melakukan check IN`,
                            status: "success",
                            position: "top",
                            duration: 3000,
                            isClosable: true,
                          ...
                          return toast({
                            title: `Hi ${res.data.data.name}, berhasil melakukan check Out`,
                            status: "success",
                            position: "top",
                            duration: 3000,
                            isClosable: true,
